...
def plot_experiment(dataset, N, k, k_eval, p, K, noisy_dims, keep_variance, iterations, output_dim, repetitions, include_random, include_sfa, include_foreca, include_gfa1, include_gfa2, N2, cachedir, processes, use_test_set=True, x_offset=0, y_label=True, legend=True, plot_time=False, seed=0):
    ...
    if include_random:
        ...
        plt.errorbar(x=x, y=m, yerr=s, linewidth=1.2, elinewidth=.5, color='green', ecolor=cc.to_rgba('green', alpha=ecolor_alpha), marker=None, linestyle=':')
    ...
    if include_foreca:
        ...
        s = np.std(values, axis=-1)
        x = np.array(result.iter_args[iter_arg]) - x_offset
        plt.errorbar(x=x, y=m, yerr=s, linewidth=1.2, elinewidth=.5, color='red', ecolor=cc.to_rgba('red', alpha=ecolor_alpha), marker=None, linestyle='-.')
...
#        legends.append('ForeCA')
    ...
    for i, _ in enumerate(result.iter_args['algorithm']):
        ...
        plt.errorbar(x=x, y=m, yerr=s, linewidth=1.2, elinewidth=.5, color='red', ecolor=cc.to_rgba('red', alpha=ecolor_alpha), markerfacecolor=facecolors[i], marker=markers[i], linestyle=linestyles[i], markersize=10)
    legends += ['PFA']
    ...
    if include_gfa1:
        ...
        for i, _ in enumerate(result.iter_args['algorithm']):
            ...
            s = np.std(values[i], axis=-1)
            x = np.array(result.iter_args[iter_arg]) + x_offset
            plt.errorbar(x=x, y=m, yerr=s, linewidth=1.2, elinewidth=.5, color='blue', ecolor=cc.to_rgba('blue', alpha=ecolor_alpha), markerfacecolor=facecolors[i], marker=markers[i], linestyle=linestyles[i], markersize=7)
        legends += ['GPFA (1)']
