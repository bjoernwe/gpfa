...
if __name__ == '__main__':
    ...
    for d, dim in enumerate(iter_dims):
        ...
        for i, iter in enumerate(iterations):
            print dim, 'x', iter
            result_without_noise = eb.experiment(algorithm='fpp', k=k, iterations=iter, iteration_dim=dim, reduce_variance=do_pca, additional_noise_dim=0, additional_noise_std=0, additive_noise=0)
            for r in range(trials):
                result_with_noise    = eb.experiment(algorithm='fpp', k=k, iterations=iter, iteration_dim=dim, reduce_variance=do_pca, additional_noise_dim=noisy_dims, additional_noise_std=200, additive_noise=additive_noise)
