...
if __name__ == '__main__':
    ...
    for algorithm in ['lpp', 'fpp']:
        baseline_result[algorithm] = eb.experiment(algorithm=algorithm, k=k, iterations=iterations, iteration_dim=iter_dim, reduce_variance=False, whitening=False, normalize_std=False, additional_noise_dim=0, additional_noise_std=0)
    ...
    for a, algorithm in enumerate(['lpp', 'fpp']):
        ...
        for p, params in enumerate([(False, False, False), (False, False, True), (True, False, False), (False, True, False)]):
            results[algorithm][p] = {}
            tmp_result = eb.experiment(algorithm=algorithm, k=k, iterations=iterations, iteration_dim=iter_dim, reduce_variance=params[0], whitening=params[1], normalize_std=params[2], additional_noise_dim=0, additional_noise_std=0)
